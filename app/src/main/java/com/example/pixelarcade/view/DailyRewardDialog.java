...
public class DailyRewardDialog extends Dialog {
    ...
    private void calculateStreak() {
        UserDataManager udm = UserDataManager.getInstance(getContext());
        long lastClaimTime = udm.getLong("last_daily_claim", 0);
        currentStreak = udm.getInt("streak_days", 0);
    ...
    private void claimReward(Button btnAction, TextView tvSubtitle, LinearLayout claimedCard, TextView claimedTitle) {
        ...
        data.put("coins", currentCoins + wonAmount);
        data.put("total_coins_earned", totalEarned + wonAmount);
        data.put("streak_days", currentStreak + 1);
        data.put("last_daily_claim", System.currentTimeMillis());
