...
public class GameActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        highestScore = udm.getInt("high_score_2048", 0);
        tvHighestScore.setText(String.valueOf(highestScore));

        // Read settings
        animationsEnabled = udm.getBoolean("animations", true);
        gridLinesEnabled = udm.getBoolean("grid_lines", true);

        // Increment Play Count
        int plays = udm.getInt("plays_2048", 0);
        udm.putInt("plays_2048", plays + 1);
    ...
    private void updateScoreOnly() {
        ...
        if (score > highestScore) {
            ...
            UserDataManager.getInstance(this)
                .putInt("high_score_2048", highestScore);
    ...
    private void showEndGameOverlay(boolean isWin) {
        ...
        if (isWin) {
            ...
            UserDataManager udmUndo = UserDataManager.getInstance(this);
            int coins = udmUndo.getInt("coins", 0);
            if (coins >= 20 && engine.canUndo()) {
                btnOverlayUndo.setVisibility(View.VISIBLE);
                btnOverlayUndo.setOnClickListener(v -> {
                    int currentCoins = udmUndo.getInt("coins", 0);
                    if (currentCoins >= 20) {
                        udmUndo.putInt("coins", currentCoins - 20);
                        engine.undo();
                        tileContainer.removeAllViews();
                        activeTiles = new TextView[gridSize][gridSize];
                        overlayView.setVisibility(View.GONE);
    ...
    private void awardCoins(int amount, String message) {
        UserDataManager udmAward = UserDataManager.getInstance(this);
        int coins = udmAward.getInt("coins", 0);
        int totalEarned = udmAward.getInt("total_coins_earned", 0);
        int earned2048 = udmAward.getInt("coins_earned_2048", 0);
        java.util.Map<String, Object> data = new java.util.HashMap<>();
        data.put("coins", coins + amount);
        data.put("total_coins_earned", totalEarned + amount);
        data.put("coins_earned_2048", earned2048 + amount);
        udmAward.putMultiple(data);
